...
| `sync.keep_versions` | How many non-default versions to keep per project (most recently created first) — older ones are deleted on the next sync. `0` (default) keeps everything; the default version itself is never pruned. Defaults to `env('DOCS_KEEP_VERSIONS', 0)`. |
