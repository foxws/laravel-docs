...
the current default unchanged. If no version is marked default, the newest
by version number is used (`1.10.0` above `1.9.0`, a leading `v` ignored;
names like `latest` rank below any version number), and removing the default
version with `docs:versions:remove` marks the newest remaining version as
the new default. Each version syncs independently from its
...
This package has no opinion on `metadata`'s shape, but a docs UI rendering a
"package info" sidebar next to the project's own overview page tends to
reach for the same handful of keys:
...
metadata:
  ...
  used_by:
    ...
    href: "https://stry.example.com"
---
```

`requires`/`laravel`/`runtime`/`licence` are free-form strings a "Package"
box can list alongside the project's default version. `used_by` is a
free-form object (`name` and `href` required, `desc` optional) for a "Used
by X" card — a nod to whatever product a UI wants to credit, not a concept
