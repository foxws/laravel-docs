...
Inspect what is registered with `docs:projects:list`, `docs:versions:list {project?}` and `docs:documents:list {project?} {version?}` — each prints a table linking projects, versions and documents together.
