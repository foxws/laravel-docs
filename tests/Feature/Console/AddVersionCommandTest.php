...
it('unmarks the previous default when registering a new default version', function () {
    $project = Project::factory()->create(['slug' => 'laravel-podman']);
    ...
    $newDefault = Version::query()->where('name', '2.0.0')->firstOrFail();

    expect($newDefault->is_default)->toBeTrue()
        ->and($previousDefault->refresh()->is_default)->toBeFalse();
});

it('does not change is_default when the option is omitted', function () {
    $project = Project::factory()->create(['slug' => 'laravel-podman']);
    ...
    expect($version->refresh()->is_default)->toBeTrue();
});
